...
if (isset($_POST['request'])) {
    ...
    if (!$post_provider || !$post_pembayaran || !$post_jumlah) {
        $_SESSION['hasil'] = array('alert' => 'danger', 'judul' => 'Permintaan Gagal', 'pesan' => 'Lengkapi Bidang Berikut:<br/> - Provider <br /> - Pembayaran <br /> - Jumlah');
        ...
        exit(header("Location: ".$config['web']['url']."invoice-emoney"));
        
    } else if ($post_jumlah < 25000) {
        $_SESSION['hasil'] = array('alert' => 'danger', 'judul' => 'Permintaan Gagal', 'pesan' => 'Minimal Deposit Emoney 25000.');

    } else if ($post_jumlah > 5000000) {
        $_SESSION['hasil'] = array('alert' => 'danger', 'judul' => 'Permintaan Gagal', 'pesan' => 'Maksimal Deposit Emoney Rp5.000.000.');
        ...
        $insert = $conn->query("INSERT INTO deposit_emoney VALUES ('','$kode', '$sess_username', '".$data_metod['tipe']."', '".$data_metod['provider']."' ,'$metodnya', '-','".$data_metod['tujuan']."','$reg', '$amount', 'Pending', 'Website', '$date', '$time')");
        if ($insert == TRUE) {
            exit(header("Location: ".$config['web']['url']."invoice-emoney"));
...
<!--Title-->
<title>Deposit via E-Money</title>
...
<div class="row">
    <div class="col-md-7">
        <div class="card">
            <div class="card-body">
                <h4 class="text-uppercase text-center header-title"><i class="ti-wallet text-primary"></i> DEPOSIT E-MONEY

                <?php
                    //INFO STATUS DEPOSIT
                    $cek_status = $conn->query("SELECT * FROM metode_depo WHERE tujuan NOT IN ('DPEDIA') AND tipe = 'EMoney' AND keterangan = 'OFF'");
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-12">
                            <select class="form-control" name="provider" id="provider">
                                ...
                                <?php
                                $cek_kategori = $conn->query("SELECT * FROM metode_depo WHERE tujuan NOT IN ('DPEDIA') AND tipe = 'EMoney' AND keterangan = 'ON' ORDER BY nama ASC");
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="table-responsive">
                    ...
                    <ol class="list-p">
                        <li>Minimal deposit Rp.25.000.</li>
