...
final class OrderService
{
    ...
    public function createPendingDigital(string $username,string $serviceProviderId,string $operator,string $target,string $noMeter='',string $source='Website'):array
    {
        ...
        try{
            $s=$this->db->prepare("SELECT id,provider_id,layanan,harga,profit,provider FROM layanan_digital WHERE provider_id=? AND status='Normal' LIMIT 1 FOR UPDATE");$s->bind_param('s',$serviceProviderId);$s->execute();$service=$s->get_result()->fetch_assoc();$s->close();if(!$service)throw new RuntimeException('Produk tidak tersedia.');$price=(int)$service['harga'];if($price<=0)throw new RuntimeException('Harga produk tidak valid.');
            $u=$this->db->prepare("SELECT id,username,saldo FROM users WHERE username=? AND status='Aktif' LIMIT 1 FOR UPDATE");$u->bind_param('s',$username);$u->execute();$user=$u->get_result()->fetch_assoc();$u->close();if(!$user)throw new RuntimeException('Akun tidak aktif.');if((int)$user['saldo']<$price)throw new RuntimeException('Saldo Tidak Mencukupi');
            ...
            $i=$this->db->prepare("INSERT INTO pembelian_digital (oid,provider_oid,user,layanan,harga,profit,target,no_meter,keterangan,status,date,time,place_from,provider,refund) VALUES (?,'',?,?,?,?,?,'','Pending',CURDATE(),CURTIME(),?,?,0)");$i->bind_param('sssiissss',$oid,$username,$service['layanan'],$price,$profit,$target,$noMeter,$source,$service['provider']);$i->execute();$i->close();
