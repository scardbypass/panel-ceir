...
final class OrderCatalog
{
    public static function schema(?string $json): array
    {
        ...
        foreach ($data as $field) {
            ...
            if (!in_array($type, ['text', 'number', 'tel', 'email', 'textarea', 'select'], true)) {
                $type = 'text';
            }
            ...
            $fields[] = [
                ...
                'min' => isset($field['min']) ? max(0, (int)$field['min']) : null,
                'max' => isset($field['max']) ? max(0, (int)$field['max']) : null,
                'options' => $options,
            ];
        }
    ...
    {
        $items = [];
        $sql = "SELECT m.service_id,m.label,m.icon,m.group_name,m.sort_order,
                       l.layanan,l.harga,l.status,l.provider_id,l.provider,l.image_url
                FROM order_menu m
                JOIN layanan_digital l ON l.provider_id=m.service_id
                WHERE m.is_visible=1 AND l.status='Normal'
    ...
    public static function service(mysqli $db, string $serviceId): ?array
    {
        ...
        // Use escaped SQL instead of mysqli_stmt::get_result(), which requires mysqlnd.
        $safeId = $db->real_escape_string($serviceId);
        $q = $db->query("SELECT * FROM layanan_digital WHERE provider_id='{$safeId}' AND status='Normal' LIMIT 1");
        if (!$q) return null;
