...
if (isset($_POST['update'])) {
    ...
    $deponya = $conn->query("SELECT * FROM deposit_emoney WHERE kode_deposit = '$get_id'");
    ...
    if ($deponya->num_rows == 0) {
        ...
        if ($conn->query("UPDATE deposit_emoney SET status = '$status'  WHERE kode_deposit = '$get_id'") == true){
            if ($status == "Success") {
    ...
    $get_id = $conn->real_escape_string($_GET['id_deposit']);
    $cek_deponya = $conn->query("SELECT * FROM deposit_emoney WHERE kode_deposit = '$get_id'");

    if ($cek_deponya->num_rows == 0) {
        $_SESSION['hasil'] = array('alert' => 'danger', 'judul' => 'Permintaan Gagal', 'pesan' => 'Data Deposit Belum Ditemukan');
    } else {
        if ($conn->query("DELETE FROM deposit_emoney WHERE kode_deposit = '$get_id'") == TRUE) {
...
<div class="row">
    <div class="col-md-12">
        <br /><h2 class="text-center">Manajemen dan Laporan Data Deposit Emoney Dana Member</h2><br/>
    </div>
</div>
<!-- End-Page-Title -->
        ...
        <div class="card widget-box-three">
            <div class="card-body">
                ...
                <div class="text-right">
                    <p class="text-uppercase font-14 m-b-10 font-600">Total Deposit Emoney</p>
                    <h3 class="m-b-10"><span>Rp <?php echo number_format($data_deposit_emoney['total'],0,',','.'); ?> Dari <?php echo $jumlah_deposit_emoney_member; ?> Deposit</span></h3>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <tbody>
                            <?php 
                                ...
                                if (isset($_GET['cari'])) {
                                    ...
                                    $cek_depo = "SELECT * FROM deposit_emoney WHERE kode_deposit LIKE '%$cari_oid%' ORDER BY id DESC"; // edit
                                    } else {
                                    $cek_depo = "SELECT * FROM deposit_emoney ORDER BY id DESC"; // edit
                                    }
                                    if (isset($_GET['cari'])) {
                            ...
                            <tr>
                                <form action="<?php echo $_SERVER['PHP_SELF']; ?>?id_deposit=<?php echo $data_depo['kode_deposit']; ?>" class="form-inline" role="form" method="POST"> 
                                    ...
                                    <td align="center">
                                        <a href="javascript:;" onclick="deposit('<?php echo $config['web']['url'];?>admin-dashboard/ajax/deposit/view-deposit-emoney?kode_deposit=<?php echo $data_depo['kode_deposit']; ?>')" class="btn btn-sm btn-primary"><i aria-hidden="true" class="fa fa-eye" title="view"></i></a>
