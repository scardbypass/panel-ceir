...
if (!$service) { http_response_code(404); exit('Produk tidak ditemukan atau sedang tidak tersedia.'); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pesan'])) {
    require __DIR__ . '/../lib/session_login.php';
    $values = [];
    try {
        foreach ($fields as $field) {
            $name = $field['name'];
            $value = trim((string)($_POST[$name] ?? ''));
            if ($field['required'] && $value === '') throw new InvalidArgumentException($field['label'] . ' wajib diisi.');
            if ($field['type'] === 'tel') $value = preg_replace('/\D+/', '', $value);
            if ($value !== '' && $field['min'] !== null && $field['type'] !== 'text' && strlen($value) < $field['min']) throw new InvalidArgumentException($field['label'] . ' terlalu pendek.');
            if ($value !== '' && $field['max'] !== null && strlen($value) > $field['max']) throw new InvalidArgumentException($field['label'] . ' terlalu panjang.');
            if ($field['type'] === 'email' && $value !== '' && !filter_var($value, FILTER_VALIDATE_EMAIL)) throw new InvalidArgumentException($field['label'] . ' tidak valid.');
            if ($field['type'] === 'select' && $value !== '' && !in_array($value, $field['options'], true)) throw new InvalidArgumentException('Pilihan ' . $field['label'] . ' tidak valid.');
            $values[$name] = $value;
        }
        $target = '';
        foreach ($fields as $field) if (($values[$field['name']] ?? '') !== '') { $target = (string)$values[$field['name']]; break; }
        ...
        if ($provider === 'ceirgo') {
            $stmt = $conn->prepare("SELECT api_key,link FROM provider WHERE LOWER(code)='ceirgo' LIMIT 1"); $stmt->execute(); $cfg=$stmt->get_result()->fetch_assoc(); $stmt->close();
            if (!$cfg || trim((string)$cfg['api_key'])==='') throw new RuntimeException('Provider CEIRGo belum dikonfigurasi.');
            $params = [];
            foreach ($values as $key=>$value) {
                if ($value === '') continue;
                if (strtolower($key)==='imei') $params['imeis'] = [$value];
                elseif (strtolower($key)==='imeis') $params['imeis'] = array_values(array_filter(preg_split('/[,\s]+/', $value)));
            ...
            $stmt = $conn->prepare("SELECT link,api_id,api_key,status FROM provider WHERE UPPER(code)='DHRU' LIMIT 1"); $stmt->execute(); $cfg=$stmt->get_result()->fetch_assoc(); $stmt->close();
            if (!$cfg || strtolower((string)($cfg['status']??'active'))==='disabled') throw new RuntimeException('Provider DHRU sedang dinonaktifkan.');
            $client = new DhruClient((string)$cfg['link'], (string)$cfg['api_id'], (string)$cfg['api_key']);
            $params = ['ID'=>(string)$service['provider_id']];
            foreach ($values as $key=>$value) if ($value !== '') $params[strtoupper($key)] = $value;
        ...
        $_SESSION['hasil']=['alert'=>'success','judul'=>'Order berhasil','pesan'=>'<b>Order ID:</b> '.htmlspecialchars($oid).'<br><b>Produk:</b> '.htmlspecialchars($service['layanan']).'<br><b>Total:</b> Rp '.number_format((int)$order['price'],0,',','.').'<br><b>Status:</b> Processing'];
        header('Location: /riwayat/pemesanan-digital'); exit;
    } catch (Throwable $e) {
        if (!empty($oid)) { try { (new OrderService($conn))->markFailed($oid, $e->getMessage()); } catch (Throwable $ignored) {} }
        $_SESSION['hasil']=['alert'=>'danger','judul'=>'Order gagal','pesan'=>htmlspecialchars($e->getMessage())];
    }
}
require_once __DIR__ . '/../lib/header.php';
?>
<title><?=htmlspecialchars($service['layanan'])?> — Order</title>
<meta name="description" content="Order <?=htmlspecialchars($service['layanan'])?> dengan proses otomatis.">
<style>
...
<div class="order-v3">
 <div class="order-hero"><div class="order-kicker">Digital Service</div><h1><?=htmlspecialchars($service['menu_label'] ?: $service['layanan'])?></h1><p>Form order dibuat otomatis dari konfigurasi produk. Pilih data yang diperlukan, lalu kirim sekali.</p></div>
 <div class="order-grid">
  <div class="order-card"><div class="order-body"><form method="post" id="orderForm"><input type="hidden" name="csrf_token" value="<?=htmlspecialchars($config['csrf_token']??'')?>"><input type="hidden" name="service_id" value="<?=htmlspecialchars($service['provider_id'])?>">
   <?php foreach($fields as $f): ?><div class="field"><label><?=htmlspecialchars($f['label'])?><?= $f['required']?' *':'' ?></label><?php if($f['type']==='textarea'): ?><textarea class="form-control" name="<?=htmlspecialchars($f['name'])?>" placeholder="<?=htmlspecialchars($f['placeholder'])?>" <?= $f['required']?'required':'' ?>></textarea><?php elseif($f['type']==='select'): ?><select class="form-control" name="<?=htmlspecialchars($f['name'])?>" <?= $f['required']?'required':'' ?>><option value="">Pilih <?=htmlspecialchars($f['label'])?></option><?php foreach($f['options'] as $o): ?><option value="<?=htmlspecialchars($o)?>"><?=htmlspecialchars($o)?></option><?php endforeach; ?></select><?php else: ?><input class="form-control" type="<?=htmlspecialchars($f['type'])?>" name="<?=htmlspecialchars($f['name'])?>" inputmode="<?= $f['type']==='tel'?'numeric':'text' ?>" placeholder="<?=htmlspecialchars($f['placeholder'])?>" <?= $f['min']!==null?'minlength="'.(int)$f['min'].'"':'' ?> <?= $f['max']!==null?'maxlength="'.(int)$f['max'].'"':'' ?> <?= $f['required']?'required':'' ?>><?php endif; ?></div><?php endforeach; ?>
  ...
  <div class="order-card"><div class="order-body"><h4 class="mb-1">Informasi Produk</h4><p class="muted mb-3"><?=htmlspecialchars($service['catatan']??'')?>&nbsp;</p><div class="info-item"><span class="info-icon"><i class="mdi mdi-tag-outline"></i></span><div><b>Service ID</b><div class="muted"><?=htmlspecialchars($service['provider_id'])?></div></div></div><div class="info-item"><span class="info-icon"><i class="mdi mdi-server-outline"></i></span><div><b>Provider</b><div class="muted"><?=htmlspecialchars(strtoupper($service['provider']))?></div></div></div><div class="info-item"><span class="info-icon"><i class="mdi mdi-shield-check-outline"></i></span><div><b>Status</b><div class="muted">Tersedia & siap dipesan</div></div></div><div class="info-item"><span class="info-icon"><i class="mdi mdi-alert-circle-outline"></i></span><div><b>Periksa data</b><div class="muted">Pastikan data yang dimasukkan benar sebelum menekan Order.</div></div></div></div></div>
 </div>
</div>
